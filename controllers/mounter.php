 ...
 * @since  1.6
 */
class Gm_ceilingControllerMounter extends JControllerLegacy
{
	/**
	...
	public function edit()
	{
		try
		{
			...
			// Get the model.
			$model = $this->getModel('Mounter', 'Gm_ceilingModel');
		...
		try
		{
			...
			if ($user->authorise('core.edit', 'com_gm_ceiling') || $user->authorise('core.edit.state', 'com_gm_ceiling'))
			{
				...
				if (!$item)
				{
					...
					$this->setRedirect(JRoute::_('index.php?option=com_gm_ceiling&view=mounters', false));
				}
				else
				{
