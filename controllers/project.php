...
class Gm_ceilingControllerProject extends JControllerLegacy
{
    ...
    public function activate()
    {
        try {
            ...
            $ref_note_type = $jinput->get('ref_note_type', 3, 'INT');
            ...
            if (!empty($refuse_note)) {
                $this->addNote($project_id, $refuse_note, $ref_note_type);
            }
            /*-------*/
    ...
    public function approve()
    {
        try {
            ...
            /*статус*/
            $old_status = $data->propject_status;
            $data->project_status = $get_data['project_status'];
            // замеры
            ...
            if($old_status != $data->project_status && $data->project_status == 5){
                Gm_ceilingHelpersGm_ceiling::notify($change_data, 2);
            }
    ...
    public function done()
    {
        try {
            ...
            //Gm_ceilingHelpersGm_ceiling::notify($data, 2);
            Gm_ceilingHelpersGm_ceiling::createImgArchive($project_id);
    ...
    function deletePrepayment(){
        try{
            $jinput = JFactory::getApplication()->input;
            $prepaymentId = $jinput->getInt('prepayment_id');
            $prepaymentModel = Gm_ceilingHelpersGm_ceiling::getModel('project_prepayment');
            $result = $prepaymentModel->delete($prepaymentId);
            die(json_encode($result));
        }
        catch (Exception $e) {
            Gm_ceilingHelpersGm_ceiling::add_error_in_log($e->getMessage(), __FILE__, __FUNCTION__, func_get_args());
    ...
    function clearProject(){
        try {
            ...
            $projectId = $jinput->getInt('project_id');
            $projectModel = Gm_ceilingHelpersGm_ceiling::getModel('project');
            $result = $projectModel->clearProject($projectId);
            die(json_encode($result));
        } catch (Exception $e) {
            Gm_ceilingHelpersGm_ceiling::add_error_in_log($e->getMessage(), __FILE__, __FUNCTION__, func_get_args());
        }
    ...
    public function update()
    {
        try {
            $jinput = JFactory::getApplication()->input;
            $projectData = $jinput->get('project', '', 'STRING');
    ...
    public function getByClientId()
    {
        try {
            $jinput = JFactory::getApplication()->input;
            $id = $jinput->getInt('client_id');
            if (!empty($id)) {
                $projectModel = Gm_ceilingHelpersGm_ceiling::getModel('project');
                $project = $projectModel->getByClientId($id);
                die(json_encode($project));
            } else {
                die(json_encode(false));
            }
    ...
    public function getStatuses(){
        try{
            $statusesModel = Gm_ceilingHelpersGm_ceiling::getModel('statuses');
            die(json_encode($statusesModel->getData()));
        } catch (Exception $e) {
            Gm_ceilingHelpersGm_ceiling::add_error_in_log($e->getMessage(), __FILE__, __FUNCTION__, func_get_args());
        }
    }
