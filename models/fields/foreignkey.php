...
class JFormFieldForeignKey extends JFormField
{
	...
	protected function getInput()
	{
		try
		{
			...
			if($this->name == "jform[dealer_id]") {	
				$query
					->select(
						array(
							't1.'.$db->quoteName($this->key_field),
							$fk_value
						)
					)
					...
					->where('t2.'.$db->quoteName('group_id') . ' = 14');
			} else {
				$query
					->select(
						array(
			...
			if($this->table == "#__gm_ceiling_clients" || $this->table == "#__gm_ceiling_projects" || $this->table == "#__gm_ceiling_groups") {
				//Если менеджер дилера, то показывать дилерских клиентов
				//$query->where('owner = '.$user->dealer_id);
			}
