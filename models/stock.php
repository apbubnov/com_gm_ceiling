...
class Gm_ceilingModelStock extends JModelList
{
    ...
    public function updateCountGoods()
    {
        try {
            ...
            $query = "UPDATE `#_gm_ceiling_canvases` AS C " .
                "SET `count`= (SELECT COUNT(R.id) FROM `#_gm_ceiling_canvases_all` AS R WHERE R.id_canvas = C.id);";

            $query .= "UPDATE `#_gm_ceiling_components_option` AS O " .
                "SET `count`= (SELECT SUM(G.count) FROM `#_gm_ceiling_components_goods` AS G WHERE G.option_id = O.id);";
