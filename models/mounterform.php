 ...
 * @since  1.6
 */
class Gm_ceilingModelMounterForm extends JModelForm
{
	private $item = null;
		...
		try
		{
			if ($this->item === null)
			{
				...
				if ($table !== false && $table->load($id))
				{
					...
					if ($id)
					{
						$canEdit = $user->authorise('core.edit', 'com_gm_ceiling.mounter.' . $id) || $user->authorise('core.create', 'com_gm_ceiling.mounter.' . $id);
					}
					else
					{
					...
					if (!$canEdit && $user->authorise('core.edit.own', 'com_gm_ceiling.mounter.' . $id))
					{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		{
			// Get the form.
			$form = $this->loadForm('com_gm_ceiling.mounter', 'mounterform', array(
				'control'   => 'jform',
				'load_data' => $loadData
				)
			);
	...
	protected function loadFormData()
	{
		try
		{
			$data = JFactory::getApplication()->getUserState('com_gm_ceiling.edit.mounter.data', array());
	...
	public function save($data)
	{
		try
		{
			$id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('mounter.id');
			...
			if ($id)
			{
				...
				$authorised = $user->authorise('core.edit', 'com_gm_ceiling.mounter.' . $id) || $authorised = $user->authorise('core.edit.own', 'com_gm_ceiling.mounter.' . $id);
			}
			else
			{
	...
	public function delete($data)
	{
		try
		{
			...
			if (JFactory::getUser()->authorise('core.delete', 'com_gm_ceiling.mounter.' . $id) !== true)
			{
