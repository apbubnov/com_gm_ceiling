...
/*                ->select('DATE_FORMAT(a.project_mounting_date, \'%d.%m.%Y %H:%i\') AS mounting_date')
    ...
    public function getMountsAndDayoffsByDealerId($dealer_id){
        try
        {
            ...
            $query->where("`u`.`dealer_id` = $dealer_id AND (`g`.`group_id` = 11 OR `g`.`group_id` = 14) AND (`p`.`project_status` > 4 OR `p`.`project_status` IS NULL) AND (`m`.`date_time` > '$currentDate' OR `d`.`date_to` > '$currentDate')");
            $query->group('`u`.`id`');
            $db->setQuery($query);
            $result = $db->loadObjectList();
            return $result;
