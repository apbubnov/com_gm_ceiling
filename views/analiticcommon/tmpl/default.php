...
<form action="" method="post" name="adminForm" id="analiticForm">
	...
	<table id = "c_analitic-table" class = "analitic-table">
		<thead class = "caption-style-tar">
			<th>Реклама</th>
            <th data-value = "all">Всего</th>
            <th data-value = "(20)">Дилеры</th>
            <th data-value = "(21)">Реклама</th>
		...
		<?php foreach ($c_items as $item) {?>
			<tr data-value = "<?php echo $item->name;?>" >
				...
				<td>
					<?php
						$c_all_dealers+= $item->dealers; 
						echo $item->dealers;
					?>
				</td>
				<td>
					<?php 
						$c_all_advt+=$item->advt;
						echo $item->advt;
					?>
				</td>
				<td>
					<?php 
						$c_all_refuse+=$item->refuse;
						echo $item->refuse;
					?>
				</td>
				<td>
		...
		<tr data-value = "total" >
				...
				<td><b><?php echo $c_all_dealers;?></b></td>
				<td><b><?php echo $c_all_advt;?></b></td>
				<td><b><?php echo $c_all_refuse;?></b></td>
...
<form action="" method="post" name="adminForm" id="d_analiticForm">
	...
	<table id = "d_analitic-table" class="analitic-table">
		<thead class = "caption-style-analitic">
            <tr>
                <th rowspan = "2">Реклама</th>
                <th data-value = "all" rowspan = "2">Всего</th>
				<th data-value = "(20)" rowspan = "2">Дилеры</th>
				<th data-value = "(21)" rowspan = "2">Рек-ма</th>
				<th data-value = "(15)" rowspan = "2">Отказ от сотр-ва</th>
                <th colspan = "3">
                    Замеры
                </th>
                <th colspan = "3">
		...
		<?php foreach ($d_items as $item) {?>
			<tr data-value = "<?php echo $item->name;?>">
				...
				<td>
					<?php
						$d_all_dealers+= $item->dealers;
					 	echo $item->dealers;
					 ?>
				</td>
				<td>
					<?php
						$d_all_advt+=$item->advt;
						echo $item->advt;
					?>
				</td>
				<td>
		...
		<tr data-value = "total">
				...
				<td><b><?php echo $d_all_dealers;?></b></td>
				<td><b><?php echo $d_all_advt;?></b></td>
				<td><b><?php echo $d_all_refused;?></b></td>
...
<script>
	 jQuery(document).ready(function(){
		...
		c_all_count['dealers'] = <?php echo $c_all_dealers;?>;
		c_all_count['advt'] = <?php echo $c_all_advt;?>;
        c_all_count['refuse'] = <?php echo $c_all_refuse;?>;
		...
		d_all_count['all_dealers'] = <?php echo $d_all_dealers;?>;
		d_all_count['all_advt'] = <?php echo $d_all_advt;?>;
		d_all_count['all_closed'] = <?php echo $d_all_closed;?>;
		d_all_count['all_mounts'] = <?php echo $d_all_mounts;?>;
		d_all_count['all_refused'] = <?php echo $d_all_refused;?>;
   ...
   /*  jQuery("#d_analitic-table tbody tr").click(function(event){
	...
	function fill_table(date1,date2,table_name){
			...
			jQuery.ajax({
                    ...
                    success: function (data) {
						...
						for(var i in all_count){
							jQuery(table_name+' > tbody > tr:last').append('<td> <b>'+all_count[i]+'<b></td>');
