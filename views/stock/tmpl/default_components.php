...
<?= parent::getPreloaderNotJS(); ?>
    ...
    <h1>Инвентаризация компонентов</h1>
    <form class="inventory" action="javascript:SendTable();"
        ...
        <table class="Elements">
            ...
            <tr>
                ...
                <td class="Article"></td>
                <td class="Count"></td>
                <td><input placeholder="Введите значение:" name="Count" pattern="[-+]{0,1}\d+|[-+]{0,1}\d+[,\.]\d+"
                           autocomplete="off"></td>
            </tr>
            </tbody>
        </table>
    ...
    <script>
            ...
            jQuery.ajax({
                ...
                success: function (data) {
                    ...
                    $.each(data.components, function (i, item) {
                        ...
                        TR.find(".Count").text(item.Count + " " + item.Unit);
                    ...
                    noty({
                        ...
                        timeout: 1500,
                        type: "success",
                        text: "Инвентаризация: " + DATA.INFO.page + " страница из " + (DATA.INFO.pages) + "<br>Количество: " + data.components.length
                    });
                    $(".inventory .Buttons .Info").html("Инвентаризация: " + DATA.INFO.page + " страница из " + (DATA.INFO.pages) + "<br>Количество: " + data.components.length);
        ...
        function SendTable() {
            ...
            DATA.INFO.type = 'SendComponents';
            var data = {info: DATA.INFO};
            data.components = [];
            $.each(DATA.TABLE.TBODY.find('tr'), function (i, v) {
                v = $(v);
                var id = v.find("[name='Id']").val(),
                    count = v.find("[name='Count']").val();
                data.components.push({id: id, count: count});
            });
