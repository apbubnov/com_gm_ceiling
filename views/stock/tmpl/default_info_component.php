...
$good_id = $app->input->get('good', NULL, 'INT');
$componentHistory = $model->getHistoryComponent($id);
$history = $componentHistory->history;
$info = $componentHistory->info;
?>
...
<form class="filter" action="/index.php?option=com_gm_ceiling&task=stock.getHistoryComponent" method="POST">
...
<h1 class="info">
    Информация по компоненту: <b><?= $info->name; ?></b>
</h1>
<table class="table">
    <thead>
    <tr>
    ...
    <tbody>
    <? foreach ($history as $h): if (empty($good_id) || $good_id == $h->good_id): ?>
        <tr>
            <td><?= $h->date_update; ?></td>
            <td><?= $h->status; ?></td>
            <td><?= $h->price; ?> руб.</td>
            <td><?= $h->count . " " . $info->unit; ?></td>
            <td><?= $h->count_now . " " . $info->unit; ?></td>
            <td><?= $h->barcode; ?></td>
            <td><?= $h->client; ?></td>
            <td><?= $h->dealer; ?></td>
            <td><?= $h->stock; ?></td>
        ...
        jQuery.ajax({
            ...
            success: function (data) {
                ...
                history.forEach(function (value) {
                    ...
                    new_tr.append(td.clone().html(value.count + " " + info.unit));
                    new_tr.append(td.clone().html(value.count_now + " " + info.unit));
