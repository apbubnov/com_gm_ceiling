    ...
    <table class="table one-touch-view" id="projectList">
        ...
        <?php foreach ($this->items as $i => $item) : ?>
            ...
            foreach ($calculations as $calculation) {
                ...
                $calculation->dealer_gm_mounting_sum = double_margin($calculation->mounting_sum, $item->gm_mounting_margin, $item->dealer_mounting_margin);
            ...
            $project_total_discount += $sum_transport_1;
            $material_sum = $canvases_sum + $components_sum;
            $mounting = $mounting_sum + $sum_transport;
...
*/
            ...
            <tr data-href="<?php echo JRoute::_('index.php?option=com_gm_ceiling&view=project&type=dealer&id=' . (int)$item->id); ?>"
                class="status<?php echo $item->project_status; ?>">
                <!-- Статус-->
                <td class="center">
                ...
                <td class="center project_sum check<?php if ($item->sum_check) {
                    echo " checked";
                } ?>" data-type="1" data-project_id="<?php echo $item->id; ?>"
                    data-value="<?php echo $project_total_discount; ?>"
                    data-new_value="<?php echo $item->new_project_sum; ?>">
                    <?php if ($item->new_project_sum > 0 && $project_total_discount != $item->new_project_sum) { ?>
                        <div class="strikeout"><?php echo $project_total_discount; ?></div>
                        <div class=""><?php echo $item->new_project_sum; ?></div>
                    <?php } else { ?>
                        <div class=""><?php echo $project_total_discount; ?></div>
                    <?php } ?>
                    <div class="sum_button"><input name="sum_checkbox[]" class="sum_checkbox" value="0" type="checkbox">
                    </div>
                </td>
                <td class="center spend_sum check<?php if ($item->cost_check) {
                    echo " checked";
                } ?>" data-type="2" data-project_id="<?php echo $item->id; ?>" data-value="<?php echo $spend; ?>"
                    data-new_value="<?php echo $material_sum; ?>">
                    <?php if ($material_sum > 0 && $material_sum != $spend) { ?>
                        <div class="strikeout"><?php echo $spend; ?></div>
                        <div class=""><?php echo $material_sum; ?></div>
                    <?php } else { ?>
                        <div class=""><?php echo $spend; ?></div>
                    <?php } ?>
                    <div class="sum_button">S</div>
                </td>
                <?php if ($item->who_mounting == 0) { ?>
                    <?php $mounting_sum = $mounting_sum; ?>
                <?php } else { ?>
                    <?php $mounting_sum = $mounting_sum; ?>
                <?php } ?>
                <td class="center mounting_sum check<?php if ($item->mounting_check) {
                    echo " checked";
                } ?>" data-type="3" data-project_id="<?php echo $item->id; ?>" data-value="<?php echo $mounting; ?>"
                    data-new_value="<?php echo $mounting_sum; ?>">
                    <?php if ($mounting_sum > 0 && $mounting_sum != $mounting) { ?>
                        <div class="strikeout"><?php echo $mounting ; ?></div>
                        <div class=""><?php echo $mounting_sum; ?></div>
                    <?php } else { ?>
                        <div class=""><?php echo $mounting; ?></div>
                    <?php } ?>
                    <div class="sum_button">S</div>
                </td>

                <td class="center earn one-touch" data-value="<?php echo $earn; ?>">
                    <?php echo $earn; ?>
                </td>
                <td class="center earn_procent one-touch <?php echo $earn_procent_class; ?>"
                    data-value="<?php echo $earn_procent; ?>">
                    <?php echo $earn_procent; ?>
                </td>
                <td class="center extra_spend_sum check<?php if ($item->spend_check) {
                    echo " checked";
                } ?>" data-type="4" data-project_id="<?php echo $item->id; ?>"
                    data-value="<?php echo $extra_spend_sum; ?>" data-new_value="<?php echo $item->new_extra_spend; ?>">
                    <?php if ($item->new_extra_spend > 0) { ?>
                        <div class="strikeout"><?php echo $spend; ?></div>
                        <div class=""><?php echo $item->new_extra_spend; ?></div>
                    <?php } else { ?>
                        <div class=""><?php echo $extra_spend_sum; ?></div>
                    <?php } ?>
                    <div class="sum_button">S</div>
                </td>
                <td class="center one-touch">
                    <?php if ($item->created == "0000-00-00") { ?>
                        -
                    <?php } else { ?>
                        <?php $jdate = new JDate($item->created); ?>
