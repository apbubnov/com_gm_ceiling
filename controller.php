...
//KM_CHANGED START
    ...
    */
    ...
    /*	функция для получения календаря монтажей в определенный день в всплывающем окне */
/*    public function get_calendar($flag = null)
    {
        try
        {
            $time = array(
            ...
            $time_arr = array();
            foreach ($time as $value) $time_arr[$value] = array();
            ...
            $query
                ->select("mounter.name AS mount, mounter.id AS  id")
                ->from($db->quoteName('#__gm_ceiling_groups', 'mounter'))
                ->where("mounter.dealer_id = " . $db->quote($dealer->id));


            $db->setQuery($query);
            $all_mount = $db->loadObjectList();

            foreach ($all_mount as $value)
                foreach ($time_arr as $key => $v)
                    $time_arr[$key][] = array('mount' => $value->mount, 'id' => $value->id);


            $jinput = JFactory::getApplication()->input;

            $date = $jinput->get('date', '0000-00-00', 'STRING');
            $mounter = $jinput->get('project_mounter', 0, 'INT');
            $db = JFactory::getDbo();
            $query = $db->getQuery(true);

            $query
                ->select("a.id, a.project_mounter")
                ->select("a.project_mounting_date")
                ->select("DATE_FORMAT(a.project_mounting_date,'%H:%i') AS time")
                ->from($db->quoteName('#__gm_ceiling_projects', 'a'))
                ->where("DATE_FORMAT(a.`project_mounting_date`,'%Y-%m-%d') = " . $db->quote($date));

            $db->setQuery($query);
            $projects = $db->loadObjectList();

            foreach ($projects as $project)
                foreach ($time_arr[$project->time] as $key => $value)
                    if ($project->project_mounter == $value['id'])
                        unset($time_arr[$project->time][$key]);

            foreach ($time_arr as $key => $value)
                if (count($value) < 1) unset($time_arr[$key]);

            if (count($time_arr) < 1) $time_arr = array("message" => 1, "info" => "Нет монтажников на этот день");
            else $time_arr = array("message" => 0, "info" => $time_arr);

            if ($flag == 1) return $time_arr;
            else die(json_encode($time_arr));
            ...
            $result .= "<tr>";
            $result .= "<th>№</th>";
            $result .= "<th>Бригада</th>";
            $result .= "<th>Начало</th>";
            $result .= "</tr>";
            foreach ($projects as $each) {
                $jdate = new JDate($each->project_mounting_from);
                $project_mounting_from = $jdate->format('d.m.Y H:i:s');

                $result .= "<tr>";
                $result .= "<td>" . $each->id . "</td>";
                $result .= "<td>" . $each->name . "</td>";
                $result .= "<td>" . $project_mounting_from . "</td>";
                $result .= "</tr>";

            }
            if (count($projects) >= 13) {
                $result .= "<tr><td colspan='4'>Нет монтажей на этот день</td></tr>";
            }
            $result .= "</table>";
            die($result);
        }
...
*/
    /*	функция для получения календаря замеров в определенный день в всплывающем окне */
/*    public function get_calendar2($flag = null)
    {
        $time = array(
            0 => "09:00",
            1 => "10:00",
        ...
        $query = $db->getQuery(true);
        $query
            ->select(array('t1.`id`', 't1.`name`'))
            ->from($db->quoteName("#__users", 't1'))
            ->join('INNER', $db->quoteName('#__user_usergroup_map', 't2') . ' ON (t1.' . $db->quoteName('id') . ' = t2.' . $db->quoteName('user_id') . ')')
            ->where('(t2.' . $db->quoteName('group_id') . ' = 22 OR t2.group_id =14) AND (t1.' . $db->quoteName('dealer_id') . ' = ' . $dealer->id . ' or t1.' . $db->quoteName('dealer_id') . ' = 0)  ORDER BY  t2.user_id DESC ');
        $db->setQuery($query);
        $all_zamer = $db->loadObjectList();

        foreach ($all_zamer as $value)
            foreach ($time_arr as $key => $v)
                $time_arr[$key][] = array('zamer' => $value->name, 'id' => $value->id);
        ...
        $query
            ->select("a.id")
            ->select("a.project_calculation_date, a.calculated_by")
            ->select("DATE_FORMAT(a.project_calculation_date,'%H:%i') AS time")
            ->select("b.name")
            ->from($db->quoteName('#__gm_ceiling_projects', 'a'))
            ->join('INNER', $db->quoteName('#__users', 'b') . ' ON (' . $db->quoteName('a.calculated_by') . ' = ' . $db->quoteName('b.id') . ')')
            ->where("DATE_FORMAT(a.`project_calculation_date`,'%Y-%m-%d') = " . $db->quote($date));
        ...
        foreach ($projects as $project)
            foreach ($time_arr[$project->time] as $key => $value)
                if ($project->calculated_by == $value['id'])
                    unset($time_arr[$project->time][$key]);

        foreach ($time_arr as $key => $value)
            if (count($value) < 1) unset($time_arr[$key]);

        if (count($time_arr) < 1) $time_arr = array("message" => 1, "info" => "Нет замерщиков на этот день");
        else $time_arr = array("message" => 0, "info" => $time_arr);
